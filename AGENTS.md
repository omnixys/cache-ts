<!-- repository: packages/ts/cache | kind: PACKAGE | stack: ts-package -->
...
Omnixys cache package for TypeScript (Redis/Valkey).

- Repository path: `packages/ts/cache` (relative to the Omnixys root)
...
Peer/@nestjs/common surface; type tests present (tsconfig.type-tests.json).
